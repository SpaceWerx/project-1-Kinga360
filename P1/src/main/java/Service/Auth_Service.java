...
public void register(Users userToBeRegistered) {
		...
		//	 Throws a null pointer exception if the user name is already taken.
			...
			userToBeRegistered.add( password, roles);
			User_Services userService = new User_Services();
			user = userService.add(userToBeRegistered);
...
//	// Try catch block will catch any exceptions thrown by the UserDAO method.
	  try {
		//  Retrieving the user data from the database from the user name given.
		  user = User_Services.getUserName(userName);
